...
const AdminSales: React.FC = () => {
  ...
  const fetchSalesData = async () => {
    try {
      ...
      const response = await fetch(`${API_URL}/api/admin/sales?${params.toString()}`, {
        credentials: 'include'
      });
  ...
  const fetchTransactions = async (page = 1) => {
    try {
      ...
      const response = await fetch(`${API_URL}/api/admin/transactions/sales?${params}`, {
        credentials: 'include'
      });
  ...
  const downloadReport = async (format: 'excel' | 'pdf') => {
    try {
      ...
      console.log('Full URL:', `${API_URL}/api/admin/sales/download?${params}`);

      const response = await fetch(`${API_URL}/api/admin/sales/download?${params}`, {
        credentials: 'include',
        method: 'GET'
      });
      ...
      const dateSuffix = dateFilter.start && dateFilter.end 
        ...
        : selectedPeriod;
      a.download = `sales-report-${dateSuffix}-${new Date().toISOString().split('T')[0]}.${format === 'excel' ? 'xlsx' : 'pdf'}`;
  ...
  const downloadTransactionDetails = async (format: 'excel' | 'pdf') => {
    try {
      ...
      const response = await fetch(`${API_URL}/api/admin/sales/download?${params.toString()}`, {
        credentials: 'include',
      });
          ...
          {/* Transaction Details in Overview */}
            <Card>
              ...
              <div className="flex items-center justify-between">
                ...
                <div className="flex gap-2">
                  <Button
                    onClick={() => downloadTransactionDetails('excel')}
                    variant="outline"
                    size="sm"
                    className="flex items-center gap-2"
                    disabled={isExporting}
                  ...
                  <Button
                    onClick={() => downloadTransactionDetails('pdf')}
                    variant="outline"
                    size="sm"
                    className="flex items-center gap-2"
                    disabled={isExporting}
